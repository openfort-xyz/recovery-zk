...
interface IRecoveryManager {
    ...
    event RecoveryStarted(
        bytes32 indexed intentHash, address indexed wallet, IKey.KeyDataReg newOwner, uint256 deadline
    );
    ...
    event ProofSubmitted(bytes32 indexed intentHash, bytes32 indexed guardianIdentifier, uint256 approvalCount);
    ...
    event RecoveryExecuted(bytes32 indexed intentHash, address indexed wallet, IKey.KeyDataReg newOwner);
    ...
    function getSession()
        ...
        returns (
            bytes32 intentHash,
            IKey.KeyDataReg memory newOwner,
            uint256 deadline,
