...
contract RecoveryManager is IRecoveryManager, IKey {
    // ============ Errors ============
    ...
    KeyDataReg private _sessionNewOwner;
        ...
        override
    {
        ...
        if (intent.newOwner.limits != 0) revert InvalidIntent();
        if (bytes32(intent.newOwner.key) == bytes32(0)) revert InvalidIntent();
        if (intent.newOwner.keyControl != KeyControl.Self) revert InvalidIntent();
    ...
    function executeRecovery() external override {
        ...
        KeyDataReg memory newOwner = _sessionNewOwner;
        ...
        IRecovery(wallet).setOwner(newOwner);
    ...
    function cancelRecovery() external override {
        ...
        if (msg.sender != IRecovery(wallet).owner()) revert Unauthorized();
